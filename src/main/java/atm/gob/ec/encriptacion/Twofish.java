/*
 Nombre: Twofish_Properties.java
 Fecha de creacion: 
 Autor: Internet
 Descripcion: Esta clase es usada por la clase de encriptacion Twofish
 Fecha de modificacion: 
 Autor modificacion: Erik Flores
 Observaciones modificacion: Se modifica esta clase de manera que la semilla de
 generacion aleatoria sea una variable de clase con
 valor EPMTGEP. Tambi�n se la introdujo en el
 ...
 *
 * Reference:<ol> <li>TWOFISH2.C -- Optimized C API calls for TWOFISH AES
 * submission, Version 1.00, April 1998, by Doug Whiting.</ol><p>
 *
 * <b>Copyright</b> &copy; 1998 <a href="http://www.systemics.com/">Systemics
 ...
 */
public final class Twofish extends Twofish_Properties // implicit no-argument constructor
{
// Debugging methods and variables
//...........................................................................

    private static String semilla = "EPMTGEP";
...
//...........................................................................
    ...
    private static byte[] toByte(String sa) {
        sa = sa.toUpperCase();
        char[] arreglo = sa.toCharArray();
        byte[] result = null;
        String hex = "0123456789ABCDEF";
        if (arreglo.length % 2 != 0) {
            return null;
        }
        result = new byte[arreglo.length / 2];
        for (int i = 0, j = 0; i < arreglo.length; j++, i += 2) {
            int hi = hex.indexOf(arreglo[i]);
            int lo = hex.indexOf(arreglo[i + 1]);
            result[j] = (byte) (hi * 16 + lo);
//     System.out.println("hi "+hi+" lo"+lo+" 0x"+arreglo[i]+arreglo[i+1]+" "+result[j]);
        }
        return result;
    }
//   static final String enc="ISO-8859-1";
//   static final String enc=null;
    static final String enc = null;

    public static byte[] encriptar(String bloque, String vkey) {
        int keysize = 32;
        if (DEBUG) {
            trace(IN, "self_test(" + keysize + ")");
        }
        ...
        try {
            ...
            byte[] bloque_src;
            if (enc != null) {
                kb_src = vkey.getBytes(enc);
                bloque_src = bloque.getBytes(enc);
            } else {
                kb_src = vkey.getBytes();
                bloque_src = bloque.getBytes();
            }
            byte[] kb = new byte[keysize];
            byte[] pt = new byte[BLOCK_SIZE];
            int i;
            ...
            for (i = 0; i < kb_src.length; i++) {
                kb[i] = kb_src[i];
            }
            for (i = 0; i < bloque_src.length; i++) {
                pt[i] = bloque_src[i];
            }
            ...
            return ct;
            /*
             * char [] tempo=new char[ct.length]; for(int hh=0; hh<ct.length;
             * hh++) { tempo[hh]=(char)ct[hh]; }
             * System.out.println(toString(pt));
             * System.out.println(toString(ct)); if(enc!=null) { return new
             * String(ct,enc); } else { return new String(ct); }
             */
        } catch (Exception x) {
            x.printStackTrace();
            if (DEBUG && debuglevel > 0) {
        ...
        /*
         * if (DEBUG && debuglevel > 0) debug("Self-test OK? " + ok); if (DEBUG)
         * trace(OUT, "self_test()");
         */
        return null;
    }

    public static byte[] desencriptar(byte[] bloque, String vkey) {
        int keysize = 32;
        if (DEBUG) {
            trace(IN, "self_test(" + keysize + ")");
        }
        ...
        try {
            ...
            byte[] cpt = blockDecrypt(pt, 0, key);
            byte[] ct = blockEncrypt(cpt, 0, key);

            ok = areEqual(pt, ct);
            /*
             * if (DEBUG && debuglevel > 6) { System.out.println("Intermediate
             * Plaintext Values (Decryption)"); System.out.println(); }
             */
            if (!ok) {
                throw new RuntimeException("Symmetric operation failed");
            }
            ...
            /*
             * System.out.println(toString(pt));
             * System.out.println(toString(cpt)); System.out.println("ok
             * ("+cpt.length+") "+ok+" "+new String(cpt));
             */
            /*
             * if(enc!=null) { return new String(cpt,enc); } else { return new
             * String(cpt); }
             */
        } catch (Exception x) {
            x.printStackTrace();
    ...
    public static double binToDec(String binary) {
        ...
        for (int i = 0; i < binary.length(); i++) {
            li_pos = (binary.length() - 1) - i;
            ld_bit = (binary.substring(li_pos, li_pos + 1).equals("1")) ? 1 : 0;
            dec += Math.pow(2, i) * ld_bit;
        }
        return dec;
    }

    protected static String encriptar(String tempo) {  //cambiar a public
        String ls_plainSize = decToBin(tempo.length());
        //System.out.println("ls_plainSize="+ls_plainSize);
        tempo = ls_plainSize + tempo;

        while (tempo.length() % MAX != 0) {
            tempo += " ";
        }
        tempo = encripta(tempo, semilla);
        //System.out.println("TEMPO=" + tempo);
        return tempo;
    }

    protected static String decriptar(String tempo) {  //cambiar a public
        tempo = decripta(tempo, semilla);
        String ls_size = tempo.substring(0, 8);
        int li_size = 8 + (int) binToDec(ls_size);
        tempo = tempo.substring(8, li_size);
        return tempo;
    }

    public static String encripta(String plainText, String key) {
        String cipherText = "";
        String textBlock = "";
        String keyBlock = "";
        int keyRotationPos = 0;
        int beforePos = 0;
        int blockSize = BLOCK_SIZE;
        int afterPos = beforePos + blockSize;

        int i = 0;
//  System.out.println("----------------------------");
//  System.out.println("obtener key para "+plainText);
//  key=hashKey(key);
        while (beforePos < plainText.length()) {
//   System.out.println("beforePos "+beforePos);
//   System.out.println("afterPos "+afterPos+" => "+afterPos);
//   if(afterPos < plainText.length()) afterPos=plainText.length();
...
//   System.out.println("encriptar("+toString(textBlock.getBytes())+","+keyBlock+")");
            byte[] cipherByte = encriptar(textBlock, keyBlock);
            cipherText += toString(cipherByte);

//   cipherText+=blockEncrypt(textBlock,keyBlock);
            beforePos = afterPos;
            afterPos = beforePos + blockSize;
//   afterPos=afterPos+blockSize;
//   System.out.println((i++)+" => "+cipherText);
//   if(i>10) break;
//   key=hashKey(key);
        }
        return cipherText;
    ...
    public static String decripta(String plainText, String key) {
        String cipherText = "";
        String textBlock = "";
        String keyBlock = "";
        int keyRotationPos = 0;
        int beforePos = 0;
        int blockSize = 2 * BLOCK_SIZE;
        int afterPos = beforePos + blockSize;
        int i = 0;
//  System.out.println("----------------------------");
//  System.out.println("obtener key para "+plainText);
...
//   if(afterPos < plainText.length()) afterPos=plainText.length();
            ...
            textBlock = plainText.substring(beforePos, afterPos);
            keyBlock = key.substring(keyRotationPos, key.length());
            while (keyBlock.length() < textBlock.length() / 2) {
                keyBlock += key;
            }
            keyBlock = keyBlock.substring(0, textBlock.length() / 2);
            keyRotationPos = keyBlock.length() % key.length();
//   System.out.println("desencriptar("+textBlock+","+keyBlock+")");
            byte[] cipherByte = desencriptar(toByte(textBlock), keyBlock);
...
//   key=hashKey(key);
        ...
        return new String(toByte(cipherText));
    }
...
//...........................................................................
    public static void main(String[] args) {
        ...
        String ls_string0, ls_string1, ls_string2;
        byte[] bloque1;
        byte[] bloque2;
        
        try {
            ls_string0 = "NSVDLMSVCEBEMROM12#21";
//  ls_string0="09876543210987654321";
            // ls_string0="012345678901234567890";
            ls_string1 = encriptar(ls_string0);
            System.out.println("resultado 0 :" +ls_string1);
            ls_string2 = decriptar(ls_string1);
            //ls_string1 = "96EB91F2BBF56F54840E9C95AF5C4242";
            ls_string2 = decriptar(ls_string1);

            System.out.println("resultado 2 :" + ls_string2);
            System.out.println("---------");
            // System.out.println("ARGS[0] => "+args[0]);
            System.out.println("ENCRIPTAR => " + ls_string1);
            System.out.println("LONGITUD DE LA ENCRIPTACION => " + ls_string1.length());
            System.out.println("---------");
            System.out.println("CODIGO A DESENCRIPTAR => " + ls_string1);
            System.out.println("DESENCRIPTAR => " + ls_string2);
            //System.out.println(ls_string0+" => "+ls_string1);
            //System.out.println(ls_string1+" => "+ls_string2);
            //System.out.println(ls_string0+" => "+new String(toByte(ls_string1)));
            //System.out.println(new String(toByte(ls_string1))+" => "+ls_string2);
            System.out.println("");
            /*
             * ls_string0="hol�0123456789";
             * bloque1=encriptar(ls_string0,ls_string0);
             * bloque2=desencriptar(bloque1,ls_string0); ls_string2=new
             * String(bloque2); System.out.println("resultado");
             * System.out.println("---------"); System.out.println(ls_string0+"
             * => "+ls_string1); System.out.println(ls_string1+" =>
             * "+ls_string2); System.out.println(""); String ls_bloque1=new
             * String(bloque1); System.out.println(bloque1);
             * System.out.println(toString(bloque1));
             * System.out.println(toString(toByte(toString(bloque1))));
             * ls_bloque1=new String(toByte(toString(bloque1)));
